...
window.onload = function LoadFromDb() {
    $.ajax({
        ...
        success: function(data) {
            for (var i in data) {
                CreateCompany(data[i].id, data[i].name, data[i].companyPrice, data[i].parentId);
            }
        }
    });
};
...
<<<<<<< HEAD
            for(var i = 0; i < elements.length; ++i) {
                if(elements[i].className == "company")
                    elements[i].parentNode.removeChild(elements[i]);
=======
            for (var i = 0; i < elements.length; ++i) {
                if (elements[i].className == "company") elements[i].parentNode.removeChild(elements[i]);
>>>>>>> 3f16b365fd3deac2e69c64a49fde5c4b6d78beb9
            }
        }
    });
}
...
var AddCompany = function AddCompany(id, addOrEdit) {
    ...
    $.ajax({
        ...
        success: function(data) {
            addOrEditBool ? CreateCompany(data.id, companyName, price, id) : EditCompany(id, companyName, price);
        }
    });
};
...
var EditCompany = function EditCompany(id, companyName, price) {
    ...
    var priceComp = document.getElementsByName(id);
    priceComp.innerHTML = price;
};
