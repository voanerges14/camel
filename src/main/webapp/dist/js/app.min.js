...
var AddCompany = function AddCompany(id, addOrEdit) {
    ...
    var url = "";
    if (addOrEdit === undefined) url = '/add/company';else url = '/edit/company';
    $.ajax({
        type: "POST",
        contentType: "application/json",
        url: url,
        data: JSON.stringify({ id: id, name: name, income: income }),
        success: function success(newId) {
            CreateCompany(newId, name, income, id);
        }
    });
};
...
var CreateInputForm = function CreateInputForm(id, addOrEdit) {
    ...
    if (addOrEdit === undefined) node5.setAttribute("onclick", "AddCompany(this.id)");else node5.setAttribute("onclick", "AddCompany(this.id, \"edit\")");
    div.appendChild(node5);
};

var CreateCompany = function CreateCompany(id, name, income, parentId) {
    ...
    button1.setAttribute("onclick", "EditCompany(this.id)");
...
var EditCompany = function EditCompany(id) {
    CreateInputForm(id, "edit");
};
    ...
    $.ajax({
        ...
        success: function success(data) {
            for (var i = 0; i < data.length; ++i) {
                CreateCompany(data.id, data.name, data.income, data.parentId);
            }
        }
    });
}
